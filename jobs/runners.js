...
const convertLine = (string) =>
  vesta.characterArrayFromString(string).slice(0, 1)[0];
...
const sendMessage = (creds, characters) => {
  ...
  fetch(
    `https://platform.vestaboard.com/subscriptions/${creds.subId}/message`,
    {
      method: 'POST',
      headers: {
  ...
  'c5d472aa-260d-4e7e-84ab-f069a3c83729': async (data) => {
    try {
      ...
      const messageArray = [
        // Title line
        [
          65, 65, 65, 65, 65, 65, 65, 0, 0, 16, 15, 12, 12, 0, 0, 65, 65, 65,
          65, 65, 65, 65,
        ],
      ];
      ...
      if (pollInfo.voteCounts.totalVotes === 1 && !pollClosed) {
        ...
        messageArray.push(
          convertLine(
            `  closed - ${pollInfo.voteCounts.totalVotes} vote${
              pollInfo.voteCounts.totalVotes > 1 ? 's' : ''
            }  `,
          ),
        );
      } else {
        messageArray.push(
          convertLine(`       ${pollInfo.voteCounts.totalVotes} votes       `),
        );
      }
